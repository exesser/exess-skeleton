...
class ParserServiceTest extends FunctionalTestCase
{
    ...
    public function testParseWithInStatement(): void
    {
        ...
        // When
        $response = $this->parserService->parseListValue(
    ...
    public function testParseWithNeqStatement(string $expression, $expectedResult): void
    {
        ...
        // When
        $result = $this->parserService->parseListValue(
            $this->tester->grabEntityFromRepository(ListDynamic::class, ['id' => $listId]),
            $expression
        );
    ...
    public function testParseListQuery(): void
    {
        ...
        // When
        $response = $this->parserService->parseListQuery(
    ...
    public function testParseWithEnumKey(): void
    {
        ...
        $this->tester->assertSame(
            $expectedValue,
            $this->parserService->parseListValue($parserOptions, $expression)
        );
    ...
    public function testParseWithEnumValue(?string $language, string $expectedValue): void
    {
        ...
        // When
        $result = $this->parserService->parseListValue($parserOptions, '%type%');
    ...
    public function testParseSecurity(?\stdClass $baseFatEntity = null): void
    {
        $this->tester->assertSame(
            '1',
            $this->parserService->parseListValue($baseFatEntity, '%current_user_id%'),
    ...
    public function testParseNow(?\stdClass $baseFatEntity = null): void
    {
        $this->tester->assertSame(
            ...
            $this->parserService->parseListValue($baseFatEntity, '%TODAY%'),
            'Assert current date is correct'
        );
        $this->tester->assertSame(
            (new \DateTime())->format('Y-m-d H:i:s'),
            $this->parserService->parseListValue($baseFatEntity, '%NOW%'),
    ...
    public function testParseListValue(): void
    {
        ...
        $this->tester->assertEquals(
            $linkCell->getName(),
            $this->parserService->parseListValue($linkCell, '%name%')
        );

        $this->tester->assertEquals(
            $list->getName(),
            $this->parserService->parseListValue($linkCell, '%list|name%')
        );

        $this->tester->assertEquals(
            $list->getDescription(),
            $this->parserService->parseListValue($linkCell, '%list|description%')
        );

        $this->tester->assertEquals(
            [$linkCell->getName()],
            $this->parserService->parseListValue($list, "%cellLinks(name='link-name')|name%")
        );

        $this->tester->assertEquals(
            '',
            $this->parserService->parseListValue($list, "%cellLinks(name='b')|name%")
        );

        $this->tester->assertEquals(
            [$linkCell->getName()],
            $this->parserService->parseListValue($list, "%cellLinks(description='\%value\%')|name%")
        );

        $this->tester->assertEquals(
            "foo not like '%BAR%' and so on",
            $this->parserService->parseListValue($list, "foo not like '\%BAR\%' and so on")
        );
    }
    ...
    public function testParseWithBeanWithMultipleReturn(string $toBeParsedString, $expectedReturn): void
    {
        ...
        // When
        $returnValue = $this->parserService->parseListValue(
            $this->tester->grabEntityFromRepository(ListDynamic::class, ['id' => $listId]),
            $toBeParsedString
        );
...
JSON;
        ...
        // run test
        $result = $this->parserService->parseListValue($entity, $line);
...
JSON;
    ...
    public function testIfStatement(string $expression, $expected, bool $expectExceptionOnEmpty = true): void
    {
        ...
        $this->tester->assertEquals(
            $expected,
            $this->parserService->parseListValue($model, $expression),
        ...
        if ($expectExceptionOnEmpty) {
            $this->tester->expectThrowable(
                ...
                function () use ($expression): void {
                    ...
                    $this->parserService->parseListValue(new Model(), $expression);
                }
            );
        } else {
            $this->tester->assertEquals(
    ...
    {
        // When
        $result = $this->parserService->parseListValue($baseEntity, '%object_name%');
    ...
    public function testWithEntity(string $expression, $expected): void
    {
        ...
        // When
        $result = $this->parserService->parseListValue($entity, $expression);
