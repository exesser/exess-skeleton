...
describe('Form type: input', function () {
  ...
  it('should change the value of numbers', function () {
    compile();

    scope.model.person.firstName = '37.27';
    $rootScope.$apply();
    expect(inputElement.val()).toBe('37,27');

    inputElement.val('27,37').change();
    $rootScope.$apply();
  ...
  it('should let the "guidanceFormObserver" know that the values have changed when noBackendInteraction is true', function () {
    ...
    scope.fields[0].templateOptions.noBackendInteraction = false;
    $rootScope.$apply();
  });
  ...
  describe("the 'readonly' functionality", function () {
    it('should made the field readonly if the templateOptions.readonly property evaluates to true', function () {
      ...
      expect(element.find('span.unit').length).toBe(0);
      ...
      scope.fields[0].templateOptions.uom = 'kWh';
      $rootScope.$apply();

      const unitOfMeasure = $(element.find('span.unit')[0]);
      expect(unitOfMeasure.text()).toBe('kWh');
    });
  });
