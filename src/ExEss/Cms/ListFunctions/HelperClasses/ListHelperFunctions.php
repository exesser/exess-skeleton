...
use ExEss\Cms\Parser\ExpressionGroupParser;
...
class ListHelperFunctions
{
    ...
    /**
     * @param ExpressionParserOptions|mixed $baseFatEntity
     * @return mixed
    ...
    public function parseListValue(
        ...
        // if the baseFatEntity was wrapped with an options object, use it, if not, wrap it
        if ($baseFatEntity instanceof ExpressionParserOptions) {
            $parseOptions = $baseFatEntity;
        } else {
            $parseOptions = new ExpressionParserOptions($baseFatEntity);
        }
        ...
        foreach ($expressions->getExpressions() as $expression) {
            if ($expression->hasReplacement()) {
                ...
                // to stay BC, $fatEntity must be set with the last processed expression that had a replacement
                // @todo check if this can be fixed, this is only correct for a singular expression, not a group
                $fatEntity = $expression->gotReplacementFrom();
            } else {
        ...
        if (\is_array($result)) {
            ...
            $result = '';
            if ($fatEntity === null) {
                // on top of that, no fat entity found, either a name collision or a bad config
                $result = $default;
            }
        }
    ...
    public function parseListQuery(
        ...
        // unset all fat entities so the paths are cacheable
        ...
        // limit to the requested base fat entities
        $qb->andWhere($qb->expr()->in('base.id', \array_column($allBeanIds, 'id')));

        // set back all fat entity ids as it might be needed afterwards
