...
class ExpressionParser
{
    ...
    public function parse(
        ...
        // disallow path caching in case the baseFatEntity is a Model class, it makes no sense
        ...
        if (!$expression->hasPath()) {
            // verify if it might be an expression that is baseFatEntity-less
            if (($value = $this->pathResolver->resolve(null, $field, $resolverOptions)) !== null) {
                $expression->setReplacement($value);
            }
            // we are just on the baseFatEntity
            $fatEntity = $baseEntity;
        } else {
            $fatEntity = $this->pathResolver->resolve(
                $baseEntity,
                $expression->getPath(),
                $resolverOptions
            );
        ...
        if (!$expression->hasReplacement()) {
            ...
            } elseif ($fatEntity instanceof Model || !\is_iterable($fatEntity)) {
                if ($parserOptions->isQueryFormat()) {
                    $select = $resolverOptions->getLastAlias() . ".$field";
                    $expression->setReplacement($select, $fatEntity);
                } else {
                    $expression->setReplacement(
                        $this->getReplacement($fatEntity, $expression, $parserOptions),
                        $fatEntity
                    );
                }
            } else {
    ...
    /**
     * @return mixed
     * @throws ConfigInvalidException If basefatEntity is not an object.
     */
