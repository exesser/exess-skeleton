...
class Validator
{
    ...
    /**
     * @param Collection|\ExEss\Bundle\CmsBundle\Entity\Validator[] $rules
     * @param mixed $model
     ...
     * @param mixed $model
     */
    private function getParams(\ExEss\Bundle\CmsBundle\Entity\Validator $rule, $model): array
    {
        $paramArray = [
            'model' => $model,
        ];
        ...
        if ($rule->getValidator() === ValidatorType::FILE
            && $rule->getMaxFileSize() !== ''
        ...
        if (\in_array(
            $rule->getValidator(),
            [ValidatorType::EMAIL, ValidatorType::MULTI_EMAIL],
            true
        )) {
        ...
        if ($rule->getValue() !== null && $rule->getValue() !== '') {
            if ($rule->getValidator() === ValidatorType::CHOICE) {
                $paramArray['strict'] = true;
                $paramArray['choices'] = \explode('|', $rule->getValue());
            } elseif ($rule->getValidator() ===  ValidatorType::REGEX) {
